...
import { MdDashboard, MdFoodBank, MdInbox, MdOutbox, MdBarChart, MdPolicy, MdFeedback, MdSettings, MdLogout } from 'react-icons/md';
...
const LocalSidebar = ({ username }) => {
    ...
    return (
        <div className="h-screen w-96 bg-white-100 pl-8 pt-4 ">
            ...
            <nav>
                {[
                    ...
                    { name: 'Incoming Request', icon: <MdOutbox /> },
                    { name: 'Forecast', icon: <MdBarChart /> },
                    { name: 'Policies', icon: <MdPolicy /> },
                    { name: 'Complaints', icon: <MdFeedback /> }
                ].map((item) => (
                    <a href={`#${item.name.toLowerCase().replace(' ', '')}`} className="flex items-center py-2 text-gray-700 hover:bg-gray-200 rounded-md p-4 w-48" key={item.name}>
            ...
            <div className="mt-auto">
                <a href="#settings" className="flex items-center py-2 text-gray-700 hover:bg-gray-200 rounded-md p-4 w-48">
